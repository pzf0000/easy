...
#   * column:    The db column for this field. This is the same as
...
class Field(RegisterLookupMixin):
    ...
    def db_check(self, connection):
        """
        Return the db column check constraint for this field, for the
    ...
    def db_type(self, connection):
        """
        Return the db column data type for this field, for the provided
        connection.
        """
        ...
        # is that it cannot handle db column types that aren't already
        # mapped to one of the built-in Django field types. In this case, you
        # can implement db_type() instead of get_internal_type() to specify
        # exactly which wacky db column type you want to use.
        data = self.db_type_parameters(connection)
        try:
    ...
    def get_db_prep_value(self, value, connection, prepared=False):
        """
        Return field's value prepared for interacting with the db backend.
    ...
    def get_db_prep_save(self, value, connection):
        """Return field's value prepared for saving into a db."""
...
class TimeField(DateTimeCheckMixin, Field):
    ...
    def to_python(self, value):
        ...
        if isinstance(value, datetime.datetime):
            ...
            # db backend (e.g. Oracle), so we'll be accommodating.
            return value.time()
